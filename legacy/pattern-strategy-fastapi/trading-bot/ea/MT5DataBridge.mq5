...
input int      ServerPort         = 8765;              // Server port (config.yaml → server.port)
...
int OnInit()
  {
   ...
   Print("MT5DataBridge v2.20: started.");
   Print("  Server : ", base);
...
void BuildBrokerSymbolIndex()
  {
   ...
   g_bsymCount = total;
   g_bsymBuilt = true;
   if(VerboseLog) Print("MT5DataBridge: indexed ", total, " broker symbols for auto-resolution.");
  }
...
void FetchEnabledPairs()
  {
   ...
   if(httpCode != 200)
     {
      if(VerboseLog)
         Print("MT5DataBridge: pairs fetch failed HTTP=", httpCode, " error=", GetLastError());
      return;
     }
   ...
   if(start < 0 || end <= start)
     {
      if(VerboseLog) Print("MT5DataBridge: could not parse pairs response.");
      return;
     }
   ...
   for(int i = 0; i < n; i++)
     {
      ...
      if(!SymbolSelect(broker, true))
        {
         skipped++;
         if(VerboseLog) Print("MT5DataBridge: skipping unavailable symbol '", s,
      ...
      if(VerboseLog && s != broker)
         Print("MT5DataBridge: mapped '", s, "' -> broker '", broker, "'");
     }
   ...
   if(vc > 0)
     {
      ...
      g_dynCount = vc;
      if(VerboseLog)
         Print("MT5DataBridge: watching ", vc, " dashboard-enabled symbol(s), ", skipped, " skipped.");
     }
   else
     {
      g_dynCount = 0;
      if(VerboseLog)
         Print("MT5DataBridge: no usable enabled pairs — using static fallback lists.");
     }
  }
...
void PushData()
  {
   ...
   if(httpCode == 200)
     {
      if(VerboseLog)
         Print("MT5DataBridge: push OK (", bodyLen, " bytes)");
     }
   else
     {
      Print("MT5DataBridge: push failed. HTTP=", httpCode, " | error=", GetLastError());
     }
  }
...
void PushBars()
  {
   ...
   for(int s = 0; s < symCount; s++)
     {
      ...
      string symBars15 = SerializeBarsForSymbol(sym, PERIOD_M15, BarsToSend);
      string symBars30 = SerializeBarsForSymbol(sym, PERIOD_M30, BarsToSend);
      string symBars1h = SerializeBarsForSymbol(sym, PERIOD_H1,  BarsToSend);
      string symBars2h = SerializeBarsForSymbol(sym, PERIOD_H2,  BarsToSend);
      string symBars4h = SerializeBarsForSymbol(sym, PERIOD_H4,  BarsToSend);
      string symBars1d = SerializeBarsForSymbol(sym, PERIOD_D1,  BarsToSend);
   ...
   if(httpCode == 200)
     {
      if(VerboseLog)
         Print("MT5DataBridge: bars push OK (", bodyLen, " bytes)");
     }
   else
     {
      Print("MT5DataBridge: bars push failed. HTTP=", httpCode, " | error=", GetLastError());
     }
  }
...
string SerializeBarsForSymbol(const string sym, ENUM_TIMEFRAMES period, int count)
  {
   ...
   double point  = SymbolInfoDouble(sym, SYMBOL_POINT);   // for per-bar spread → price units
   ...
   for(int i = 0; i < copied; i++)
     {
      ...
      // Per-bar spread (MqlRates.spread is in POINTS) converted to PRICE units.
      // The paper bot reads this as the bar's spread so news/rollover widening
      // costs exits realistically. Safe to add: older bots ignore unknown keys.
      j += "\"sp\":" + DoubleToString(rates[i].spread * point, digits);
      j += "}";
     }
   j += "]";
   return j;
...
void PollAndExecuteCommand()
  {
   ...
   if(httpCode != 200)
     {
      if(VerboseLog)
         Print("MT5DataBridge: command poll failed HTTP=", httpCode, " error=", GetLastError());
      return;
     }

   string resp = CharArrayToString(result, 0, WHOLE_ARRAY);
   if(VerboseLog)
      Print("MT5DataBridge: command response: ", StringSubstr(resp, 0, 200));

   if(StringFind(resp, "\"command\":null") >= 0 ||
      StringFind(resp, "\"command\": null") >= 0)
     {
      if(VerboseLog) Print("MT5DataBridge: no pending commands.");
      return;
     }
   ...
   if(cmdId == "" || cmdType == "" || ticket == 0)
     {
      Print("MT5DataBridge: malformed command — id=", cmdId, " type=", cmdType, " ticket=", ticket);
      return;
     }

   Print("MT5DataBridge: executing ", cmdType, " | ticket=", ticket,
         " | new_sl=", new_sl, " | reason=", StringSubstr(reason, 0, 100));
   ...
   if(cmdType == "trail_sl" || cmdType == "move_sl_be")
     ...
     {
      Print("MT5DataBridge: unknown command type '", cmdType, "' — skipping.");
...
bool ExecuteSLModify(long ticket, double new_sl, string reason)
  {
   if(new_sl <= 0)
     {
      Print("MT5DataBridge: invalid new_sl=", new_sl, " for ticket=", ticket);
      return false;
     }

   if(!PositionSelectByTicket((ulong)ticket))
     {
      Print("MT5DataBridge: cannot select ticket=", ticket, " (may be closed). Error=", GetLastError());
      return false;
     }
   ...
   if(pos_side == "BUY" && new_sl < current_sl && current_sl > 0)
     {
      Print("MT5DataBridge: SAFETY — refusing to move BUY SL backward. current_sl=", current_sl, " new_sl=", new_sl);
      return true;
     }
   if(pos_side == "SELL" && new_sl > current_sl && current_sl > 0)
     {
      Print("MT5DataBridge: SAFETY — refusing to move SELL SL backward. current_sl=", current_sl, " new_sl=", new_sl);
      return true;
     }
   ...
   if(ok)
      Print("MT5DataBridge: SL modified ✓ ticket=", ticket, " symbol=", pos_symbol,
            " side=", pos_side, " old_sl=", current_sl, " → new_sl=", new_sl);
   else
      Print("MT5DataBridge: SL modify FAILED ticket=", ticket,
...
void AcknowledgeCommand(string cmdId)
  {
   ...
   if(httpCode == 200)
      Print("MT5DataBridge: command acked ✓ id=", cmdId);
   else
      Print("MT5DataBridge: ack failed HTTP=", httpCode, " id=", cmdId);
  }
...
string BuildPayload()
  {
   ...
   if(!HistorySelect(since, TimeCurrent()))
      Print("MT5DataBridge: HistorySelect failed, error=", GetLastError());
